# -*- coding: utf-8 -*-
# deploy mall system automatically
...
app_name = 'utopMall'
app_back_path = '/usr/local/projectback/mall'
...
app_config_file1 = 'config.properties'
...
# shutdown tomcat
print ssh.runcmd('/usr/local/tomcat/bin/shutdown.sh')
...
while not wlan_ip and num > 0:
    ...
    wlan_ip = res[0]
    print ssh.runcmd('sed -i \'s/pay_url\s*=.*/' + 'pay_url=http:\/\/' + wlan_ip + ':' + wlan_port + '\/idas\/dispatch\/dispatch.action' + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
    #print ssh.runcmd('sed -i \'s/share_rendering_url\s*=.*/' + 'share_rendering_url=http:\/\/' + wlan_ip + ':' + wlan_port + '\/idas\/share\/rendering?renderingId=' + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
    ...
    #print ssh.runcmd('sed -i \'s/icbc_pay_url\s*=.*/' + 'icbc_pay_url=http:\/\/' + wlan_ip + ':' + wlan_port + '\/idas\/icbc\/MerchantPayment' + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
    ...
    #print ssh.runcmd('sed -i \'s/xzxUrl\s*=.*/' + 'xzxUrl=http:\/\/' + wlan_ip + ':' + wlan_port + '\/html\/index.html' + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
    #print ssh.runcmd('sed -i \'s/hostNameShop\s*=.*/' + 'hostNameShop=http:\/\/' + wlan_ip + ':' + wlan_port + '\/' + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
    ...
    print ssh.runcmd('sed -i \'s/brandUrl=.*/' + 'brandUrl=http:\/\/' + hostname + ':' + idas_port + '\/idas\/wap\/shop\/brand?brandId=' + '\/' + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
    print ssh.runcmd('sed -i \'s/goodsUrl=.*/' + 'goodsUrl=http:\/\/' + hostname + ':' + idas_port + '\/idas\/wap\/goods\/detail?goodsId=' + '\/' + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
    print ssh.runcmd('sed -i \'s/shopUrl=.*/' + 'shopUrl=http:\/\/' + hostname + ':' + idas_port + '\/idas\/wap\/shop\/toShop?shopId=' + '\/' + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
    ############################调用财务系统接口地址########################
    print ssh.runcmd('sed -i \'s/finance_url=.*/' + 'finance_url=http:\/\/' + hostname + ':' + finance_port + '\/finance\/dispatch\/dispatch.action'  + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
    ############################调用IDAS接口地址########################
    print ssh.runcmd('sed -i \'s/idas_url=.*/' + 'idas_url=http:\/\/' + hostname + ':' + idas_port + '\/idas\/dispatch\/dispatch.action'  + '/g\'' + ' ' + app_config_path + '/' + app_config_file1).decode('UTF-8')
